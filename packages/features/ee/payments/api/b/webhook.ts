...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    ...
    if (!process.env.STRIPE_WEBHOOK_SECRET_B) {
      throw new HttpCode({ statusCode: 500, message: "Missing process.env.STRIPE_WEBHOOK_SECRET_B" });
    }
    const requestBuffer = await buffer(req);
    const payload = requestBuffer.toString();

    const event = stripe.webhooks.constructEvent(payload, sig, process.env.STRIPE_WEBHOOK_SECRET_B);
